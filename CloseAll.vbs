...
If (LastStatus = SessionStatusCode_Connected) Then
    ...
    If bTablesLoaded Then        
        ...
        If offers.getNextRowByColumnValue("Instrument", "EUR/USD", iter, offer) Then
            ...
            If summaries.getNextRowByColumnValue("Instrument", "EUR/USD", iter, summary) Then                                
                'Close true market order for EUR/USD
                Set reqFactory = session.getRequestFactory()
                ...
                WScript.Echo "There are no opened position for EUR/USD instrument!"            
            End if        
